...
export default function DocsEditor() {
  ...
  return (
        <div className="top-28 left-4 right-4 absolute bg-white juxstify-center items-center " >
        <h1 id="text1" className="text-black dark:text-white font-mono font-extrabold text-4xl text-center" onMouseOver={()=>{if(open)setOpen(false)}}>Note Corner : Docs</h1>
        ...
        <div className='overflow-auto  m-10 border-2 border-zinc-800 p-4 rounded flex flex-col space-y-10 justify-start items-center min-h-[20rem]'>
            ...
            <NotepadTextDashedIcon size={22} className='text-zinc-900 cursor-pointer' onClick={()=>handleAddRichTextEditor(<RichTextEditor/>)}/>
            <CodeSquare size={24} className='text-zinc-900 cursor-pointer' onClick={()=>handleAddRichTextEditor(<CodeEditor/>)}/>   
            </div>
            <div className='w-fit h-12 flex flex-row justify-center items-center z-10 cursor-pointer'onClick={()=>{}} onMouseOver={()=>{setOpen(true);}}>
                ...
                <motion.div className='flex flex-col border rounded border-zinc-300 mt-10 z-20'
                 onMouseOver={()=>{setOpen(true);}}
                    ...
                    <div className='flex flex-row items-center space-x-2 p-2 cursor-pointer'onMouseOver={()=>{setOpen(true);}} onClick={()=>handleAddRichTextEditor(<RichTextEditor/>)}><h1 className='font-mono'>Text Editor</h1><NotepadTextDashedIcon size={22} className='text-gray-800'/></div>
                    <div className='h-1 border-t border-zinc-300' onMouseOver={()=>{setOpen(true);}}></div>
                    <div className='flex flex-row items-center space-x-2 p-2 cursor-pointer' onMouseOver={()=>{setOpen(true);}} onClick={()=>handleAddRichTextEditor(<CodeEditor/>)}><h1 className='font-mono'>Code Editor</h1><CodeSquare size={24} className='text-gray-800'/></div>
                </motion.div>
            </div>
